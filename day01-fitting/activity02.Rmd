...
1. What are the dimensions of the dataset?
  The dimensions of the dataset is 1458 rows by 123 columns
  
  What does each row represent?
  it looks like each row represents a country and some summary statistics for each column.
...
2. What type of plot would you use to display the distribution of the personal freedom scores, `pf_score`? 
  ...
  We would want to use a histogram I think?
...
hist_pf_score <- hist(hfi_2016$pf_score, main = "Distribution of Personal Freedom Scores", xlab = "Personal Freedom Scores", ylab = "Frequency", col = "blue", border = "black")

hist_pf_expression_control <- hist(hfi_2016$pf_expression_control, main = "Distribution of Political Pressures and Controls on Media Content", xlab = "Expression Control Index", ylab = "Frequency", col = "red", border = "black")

```

4. Comment on each of these two distributions. 
  Be sure to describe their centers, spread, shape, and any potential outliers.
Regarding distribution of personal freedom scores we can see that the distirbution fits a pretty typical shape. We would anticipate most values to be in the center of the graph, which is true. I dont see any outliers since the values of the score is on a standard scale of 1-10

The distribution in the second set of data is a bit different. It doesnt fit the usual distribution shape we would anticipate. It seems there is no real distribution regarding political pressures and controls on media content.
...
plot(hfi_2016$pf_expression_control, hfi_2016$pf_score, 
     ...
     main = "Relationship between pf_expression_control and pf_score")
...
In summary, the intercept is the estimated value of pf_score when pf_expression_control is zero, and the slope is the estimated change in pf_score for a one-unit change in pf_expression_control. These values help quantify the relationship between the variables in the context of the linear regression.
...
8. What does this value mean in the context of this model?
  ...
  the value of rsquared should never be perfect, a model can never be perfect. A good value is anything above 70-90 and a very good value is 90-98
...
train_indices <- sample(1:nrow(hfi_2016), 0.8 * nrow(hfi_2016))

# Create the training set
train_data <- hfi_2016[train_indices, ]

# Create the testing set (20%)
test_data <- hfi_2016[-train_indices, ]
...
10. Interpret each of the estimated parameters from (10) in the context of this research question.
  That is, what do these values represent?
when expression is equal to zero we would anticipate a pf_score of 4.3 for any iteration above 0 we would anticipate an increase of .535
...
Summary statistics tell us they performed very closley to one another.
...
11. Is there any apparent pattern in the residuals plot?
  ...
  I do not see much linearity in the plot - i would say the two are not extremely coorlelated but somewhat coorelated.
...
12. Based on the histogram, does the nearly normal residuals condition appear to be violated?
  Why or why not? I would say no - the distribution of the residuals does not fit the nearly normal normal distribution.
...
13. Based on the residuals vs. fitted plot, does the constant variability condition appear to be violated?
  Why or why not?
I would say it fits the condition since there are no obscure outliers in the data set
